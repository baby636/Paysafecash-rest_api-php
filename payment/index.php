...
<title>paysafecash payment Interface</title>
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
        <div class="container">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="/">paysafecash payment Interface</a>
            </div>
...
if (count($_POST) > 0) {
    ...
    $response = $pscpayment->initiatePayment($amount, $currency, $customer_id, $customer_ip, $success_url, $failure_url, $notification_url, $correlation_id);
    ...
    if ($response == false) {
        ...
        if (isset($response["redirect"])) {
           if(empty($_POST["submitCustomer"])){
               header("Location: " . $response["redirect"]['auth_url']);
           } else {
            ?>
                    <script>
                        // Create a JS object with the customers data
                        var customerData = {
                            salutation: "mr",
                            firstName: "John",
                            lastName: "Doe",
                            birthDate: "01/02/1970",
                            phoneNumber: "+4471001234567891",
                            email: "john@doe.com",
                            address1: "76 Regent Street",
                            city: "London",
                            postalCode: "W1B 5RN"
                        };

                        // Create a hidden form containing the customers data
                        var form = document.createElement("form");
                            form.id = "psc_post_form_id";
                            ...
                            for (var i in customerData) {
                                var parameter = document.createElement('input');
                                    parameter.name = i;
                                    parameter.value = customerData[i];
                        ...
                        form.target =  ''; // '' -> redirect; '_blank' -> new tab; '{iframe_id}' 
                        form.action =   '<?php echo $response['redirect']['auth_url']; ?>';
                        form.action = form.action.replace('https://customer.test.at.paysafecard.com/psccustomer/GetCustomerPanelServlet', 'https://customer.test.at.paysafecard.com/rest/payment/panel');
                        form.submit();
                    </script>
            <?php 
           }
...
?>
        <div class="page-header">
            <h1><img src="https://www.paysafecash.com/fileadmin/UpSite/SubModulePaysafecardLayout/paysafecash/build/vectors/logo-paysafecash.svg" height="48px" /></h1>
            <p>Make a payment with paysafecash </p>
        </div>
        ...
        <div class="row">
                ...
                <div class="panel panel-default">
                    ...
                    <hr/>
                        <form method="POST">
                            ...
                            Submit Customer data: <input type="checkbox" name="submitCustomer">
                            ...
                            <br/>
                            <button type="submit" name="action" value="payment" class="btn btn-success">pay with paysafecash</button>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-sm-6">
                ...
                <p>
                    More information is available at <a href="http://www.paysafecash.com" target="_blank"> www.paysafecash.com </a>
                </p>
            </p>
